...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const measureCardOffsets = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const rawTop = rect.top + window.scrollY - currentY;
      cardTopsRef.current[i] = rawTop;
    });
    ...
    if (endElement) {
      const currentTransform = (endElement as HTMLElement).style.transform;
      let currentY = 0;
      if (currentTransform) {
        const match = currentTransform.match(/translate3d\(0px,\s*([-\d.]+)px/);
        if (match) {
      ...
      endElementTopRef.current = rect.top + window.scrollY - currentY;
    }
  }, [useWindowScroll]);
